...
class Rocket(tile: RocketTile)(implicit p: Parameters) extends CoreModule()(p)
  ...
  val (id_xcpt_raw, id_cause) = checkExceptions(List(
  ...
  // [Fix A] Mask exceptions from frozen IBuf metadata in SIMD follower mode
  ...
  div.io.req.bits.in2 := Mux(ex_reg_systolic_mul, io.systolic_opA, ex_rs(1))
  ...
  ex_reg_replay := !take_pc && ibuf.io.inst(0).valid && ibuf.io.inst(0).bits.replay && !id_systolic_follower
  ex_reg_xcpt := !ctrl_killd && id_xcpt
  ex_reg_xcpt_interrupt := !take_pc && ibuf.io.inst(0).valid && csr.io.interrupt

  when (!ctrl_killd) {
    ex_ctrl := id_ctrl
    // [Fix A] Mask RVC from frozen IBuf
    ex_reg_rvc := ibuf.io.inst(0).bits.rvc && !id_systolic_follower
    ex_ctrl.csr := id_csr
  ...
  if (rocketParams.haveSimTimeout) PlusArg.timeout(
    ...
    io.systolic_south_auto_wen  := wb_reg_valid && wb_reg_systolic_mul && !wb_xcpt
    io.systolic_south_auto_data := rf_wdata  // the MUL result being written to rd
