/*
 * gemm_tiled_simd.c — SIMD Lockstep Register-Tiled GEMM
 *
 * C[M][N] += A[M][K] * B[K][N]   (single-precision floating-point)
 *
 * All 4 cores execute in hardware SIMD lockstep, each computing a
 * disjoint slab of output rows. Uses the same register-tiled
 * micro-kernel as gemm_tiled_mimd.c for fair comparison.
 *
...
/*
 * SIMD tiled kernel — called inside SIMD region.
 * Each core reads mhartid to determine its row slab, then
 * uses the register-tiled micro-kernel.
 */
static void __attribute__((noinline)) simd_tiled_kernel(void) {
